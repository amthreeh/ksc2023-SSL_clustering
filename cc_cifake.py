...
class CoKe(nn.Module):
    ...
    def __init__(self, base_encoder, K, dim=128, num_ins=50000, num_head=10, T=0.1, dual_lr=20, stage=801, t=0.5,
                 ratio=0.4, ls=5):
    ...
    def update_center(self, epoch):
        ...
        if epoch >= self.stage:
            ...
            for i in range(0, self.num_head):
                ...
                chebyshev_distance = torch.max(torch.abs(self.pre_centers[i] - tmp_center), dim=0).values
                self.pre_centers[i] += (1. - factor) * self.pre_centers[i] + factor * tmp_center
                updated_center = F.normalize(self.pre_centers[i], dim=0)  
                expanded_chebyshev_distance = chebyshev_distance.unsqueeze(0).expand_as(updated_center)
                updated_center += (tmp_center - updated_center) * expanded_chebyshev_distance.squeeze()
