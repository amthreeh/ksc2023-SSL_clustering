    ...
    def update_center(self, epoch):
        ...
        if epoch >= self.stage:
            ...
            for i in range(0, self.num_head):
                ...
                # 유클리드 거리 대신 맨하탄 거리 또는 체비셰프 거리로 업데이트된 클러스터 중심 계산
                updated_center = F.normalize(self.pre_centers[i], dim=0)  # 여기서 normalize은 선택사항입니다.
                # print("Before unsqueeze:", manhattan_distance.shape) #[20]
                # manhattan_distance = manhattan_distance.unsqueeze(0).unsqueeze(-1)
                # print("After unsqueeze:", manhattan_distance.shape) #[20,1]
                # print("1.",updated_center.shape, "2.",(tmp_center - updated_center).shape, "After unsqueeze:", manhattan_distance.unsqueeze(0).shape)

                updated_center += (tmp_center - updated_center) * manhattan_distance.unsqueeze(0)
                # 맨하탄 거리로 업데이트된 클러스터 중심 할당
